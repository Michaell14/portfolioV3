const projectData = {
    "selected": [
        {
            "name": "Online Minesweeper Co-Op",
            "description": "A Free Online Minesweeper Game where you uncover mines and compete in real-time challenges.",
            "image": "/thumbnails/minesweeper.gif",
            "link": "https://www.minesweepercoop.com/",
            "githubLink": "https://github.com/Michaell14/Minesweeper-Co-op",
            "tags": ["Next.js", "Socket.io", "Redis"]
        },
        {
            "name": "Penn Pins",
            "description": "A social exploration app designed to help students discover campus and connect with each other in real life.",
            "image": "/thumbnails/pennpins.gif",
            "link": "https://github.com/Michaell14/explore-penn",
            "githubLink": "https://github.com/Michaell14/explore-penn",
            "tags": ["React Native", "Firebase"]
        }, 
    ],
    "projects": [
        {
            "name": "Restaurant Rating ML",
            "description": "We attempt to develop a better understanding of factors that contribute to a high/low restaurant rating score.",
            "image": "/thumbnails/5450.gif",
            "link": "https://github.com/Michaell14/5450_Final/blob/main/5450_FINAL_PROJECT.ipynb",
            "githubLink": "https://github.com/Michaell14/5450_Final/blob/main/5450_FINAL_PROJECT.ipynb",
            "tags": ["Artificial Intelligence"]
        },
        {
            "name": "Bentofy",
            "description": "This is a one-stop hub of your own life that you can share with all of those arounds you in the form of the trending Bento UI style.",
            "image": "/thumbnails/bento_screenshot.webp",
            "link": "https://bentofy.vercel.app/",
            "githubLink": "https://github.com/Michaell14/Bentofy/tree/main",
            "tags": ["In Development"]
        }, {
            "name": "Accelerometer Controller",
            "description": "A Unity game developed at UMD's Bitcamp Hackathon using an Arduino, accelerometer, and the Unity game engine",
            "image": "/bitcamp/prototype.webp",
            "link": "/projects/bitcamp",
            "githubLink": "https://github.com/Michaell14/Arduino-Controller-for-Hotdog-Unity-Game",
            "tags": ["Article", "Hardware", "Hackathon"]
        }, {
            "name": "Spotify Music Box",
            "description": "A Music box developed with the Spotify API",
            "image": "/musicbox/final_setup.webp",
            "link": "/projects/musicbox",
            "githubLink": "",
            "tags": ["Article", "Hardware"]
        }, {
            "name": "Unbe-leaf-able AI",
            "description": "Evaluated the accuracy of machine learning models in identifying 185 species of leaves using Python and Pytorch",
            "image": "/thumbnails/unbeleafable.webp",
            "link": "/projects/unbeleafable",
            "githubLink": "https://docs.google.com/presentation/d/1B_lBIkfkwzQYYXZVgo_IbSufFFa0JNr1PCqukg4qOpE/edit?usp=sharing",
            "tags": ["Article", "Artificial Intelligence"]
        }, {
            "name": "PClassic.org",
            "description": "Maintained the UI/functionality of the Philadelphia Classic's (PClassic) website.",
            "image": "/thumbnails/pclassic.webp",
            "link": "https://www.pclassic.org/",
            "githubLink": "https://github.com/pclassic",
            "tags": []
        }, {
            "name": "Encryptable",
            "description": "Visualization tool for text encryption ciphers. Created to make learning about ciphers easier for students to understand.",
            "image": "/thumbnails/encryptable_ss.webp",
            "link": "https://encryption-visualizer.vercel.app/",
            "githubLink": "https://github.com/Michaell14/encrypt-visualizer",
            "tags": []
        },
        {
            "name": "Codele",
            "description": "A parody of Wordle, but instead, you guess a random programming language!",
            "image": "/thumbnails/Codele.webp",
            "link": "https://codele.vercel.app/",
            "githubLink": "https://github.com/Michaell14/codele",
            "tags": []
        }
    ]
}

module.exports = {
    projectData: projectData["projects"],
    selectedData: projectData["selected"]
}